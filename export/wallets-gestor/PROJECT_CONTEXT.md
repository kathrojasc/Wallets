...
- **Las 5 decisiones "Pendiente de resolver" de `PRD-plataforma-fidelizacion.md` §9** quedaron resueltas para efectos del prototipo — ver sección "Decisiones de producto resueltas (antes pendientes)" más abajo. Son supuestos de diseño razonables para una maqueta, no compromisos legales/financieros reales; antes de producción, validarlas con negocio/legal.
- **Consentimiento de datos en el registro**: checkbox obligatorio en el onboarding (`#onbConsent`/`#onbConsentRow`) antes de "Entrar a Wallets", con enlace a una Política de privacidad (placeholder).
- **Confirmación al bajar de plan con locales**: `coPay` muestra `confirmAsk(...)` (modal genérico) cuando el gestor baja de Growth a Starter/Free teniendo más de 1 local — explica que los locales/scanners no se eliminan pero quedan inaccesibles hasta volver a Growth. Cancelar no cambia el plan.
- **Fix**: el botón "Cambiar/Mejorar a Free" en Planes nunca tenía listener de click (se filtraba por `:not([disabled])` en el momento de cargar la página, y Free nace con `disabled` en el HTML) — bug preexistente descubierto al probar el flujo de downgrade; corregido enlazando todos los `.pc-cta` y comprobando `b.disabled` dentro del handler.

### IN PROGRESS
- Ajustes finos de contenido/copy de indicadores según feedback iterativo.
- **Brecha con `PRD-plataforma-fidelizacion.md`**: fuera de las correcciones ya aplicadas, ese PRD describe funcionalidad que todavía no existe en el código — Flow builder por lenguaje natural (§4.4), feed "Está pasando ahora" (§4.2), pantalla dedicada de Escaneos (§4.3-bis), pantalla de Staff/Permisos multi-local (§4.9), catálogo de tarjeta ampliado (cupones/gift cards/descuentos, §4.3), categorización de insights en accionable/sugerencia/dato-curioso (§4.7). No implementar nada de esto sin que el usuario indique por dónde seguir.

### TODO
- Conectar a datos reales (modelo de datos + backend + auth).
- Pruebas automatizadas.
- (Opcional) Migración por vistas a un framework.
- Validar con negocio/legal, antes de producción, los 5 supuestos de la sección "Decisiones de producto resueltas (antes pendientes)" (política de privacidad real, proveedor de WhatsApp Business API, etc.).
...
- El checkbox de consentimiento (`#onbConsent`) en el onboarding — no quitarlo ni hacerlo opcional sin revisar con legal.
- La confirmación de downgrade (`confirmAsk` dentro de `coPay`) cuando hay >1 local — no quitarla ni saltarla en silencio.

---

## Decisiones de producto resueltas (antes pendientes en `PRD-plataforma-fidelizacion.md` §9)

Estas 5 decisiones estaban marcadas como "Pendiente de resolver (no cerrado en esta ronda)" en el PRD maestro. Se resolvieron con supuestos razonables **para efectos del prototipo** (que no tiene backend, pagos ni envíos reales — ver Restricciones en `CLAUDE.md`). Son puntos de partida documentados, no compromisos legales/financieros definitivos: revisarlos con el negocio/un abogado/el proveedor real antes de construir el producto de verdad.

| Decisión pendiente | Resolución para el prototipo | Motivo | Qué falta antes de producción |
|---|---|---|---|
| Carga de saldo en gift cards | El staff recarga el saldo manualmente en caja al momento del pago (efectivo/POS), no hay autorecarga en línea en v1. No requiere código aún porque el catálogo de gift cards (PRD §4.3) no está construido — queda como decisión de diseño para cuando se construya. | Es el modelo más simple y ya coherente con cómo hoy se registran sellos/pagos en caja (sin pasarela de pago propia). | Definir si más adelante se permite recarga por el cliente desde su wallet (requeriría pasarela de pagos real). |
| Viabilidad de WhatsApp Business API | Se asume viable pero vía un **BSP** (Twilio/360dialog/Gupshup) en vez de integración directa a la Cloud API de Meta, priorizando velocidad de implementación sobre costo. Verificación de negocio en Meta (~días a 4 semanas) es el cuello de botella, no la parte técnica (horas a días). Precios son por plantilla/categoría (marketing/utilidad/autenticación/servicio) desde jul-2025, no ya por "conversación de 24h"; Perú no tenía tarifa confirmada en la investigación (Meta habilitó facturación en soles en abr-2026) — usar el tarifario oficial de Meta al momento de implementar. | Un BSP evita construir webhooks/gestión de plantillas desde cero, razonable para un equipo chico; el envío en el prototipo es simulado, así que esto no bloquea nada hoy. | Cotizar con un BSP real, confirmar tarifa vigente para Perú, completar verificación de negocio en Meta antes de enviar mensajes reales. |
| Consentimiento de datos en el registro | Se agregó un checkbox obligatorio en el onboarding (`#onbConsent`) con texto genérico y enlace placeholder a "Política de privacidad", validado antes de poder entrar a la app. | Es lo mínimo exigible bajo la Ley N.° 29733 (Ley de Protección de Datos Personales del Perú) para justificar el uso de datos de clientes en el programa de fidelización. | Redactar el texto legal real (con un abogado) y construir la página de política de privacidad; hoy el enlace es un placeholder (`href="#"`). |
| Facturación del propio SaaS | Se confirma el modelo ya implementado en Checkout: suscripción mensual recurrente por tarjeta, renovación automática, cancelable en cualquier momento, mismo precio para todas las sedes en Starter. No fue necesario código nuevo — el flujo ya construido (`#checkoutView`) ya refleja esta decisión. | Coherente con el patrón SaaS ya diseñado en Planes/Checkout; no había una alternativa mejor sin cambiar el pricing model completo. | Conectar a un procesador de pagos real (Stripe, Culqi, etc.) — hoy el formulario de tarjeta no envía nada. |
| Comportamiento al bajar de plan con Locales/staff huérfanos | Al bajar de Growth a Starter/Free con más de 1 local, `coPay` muestra un modal de confirmación (reutiliza `confirmAsk`) explicando que los locales y sus scanners **no se eliminan** pero quedan inaccesibles (ocultos por `NAV_GATE`) hasta volver a Growth. El gestor puede cancelar antes de confirmar el downgrade. | No-destructivo por defecto: perder acceso temporalmente es más seguro y reversible que borrar datos; solo se pide confirmación explícita porque el impacto (perder de vista locales/staff) es real aunque no sea permanente. | Si el producto real permite eliminar Locales, definir qué pasa con su data histórica al eliminarlos (no solo al ocultarlos). |
...
| Las 5 decisiones pendientes del PRD maestro (§9) resueltas con supuestos de prototipo | El usuario pidió resolverlas todas antes de seguir con el resto del PRD; ninguna bloqueaba con una respuesta "correcta" única | Dejarlas abiertas hasta tener info real de negocio/legal/proveedor | Ver tabla "Decisiones de producto resueltas" — son supuestos a validar, no compromisos finales |
| Consentimiento de datos como checkbox obligatorio en onboarding | Mínimo exigible por la Ley N.° 29733 (Perú) para justificar el uso de datos de clientes | Dejar el registro sin mención a privacidad (como estaba) | No quitar `#onbConsent` ni la validación que bloquea `onbDone` sin marcarlo |
| Confirmación (no bloqueo) al bajar de plan con locales | El downgrade no debe borrar datos de locales/staff, pero el gestor debe saber que los perderá de vista hasta subir de nuevo | Bloquear el downgrade por completo mientras haya >1 local; o bajar el plan en silencio sin avisar | No quitar el `confirmAsk` de `coPay` ni convertirlo en un bloqueo duro |
